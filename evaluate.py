...
model.load_state_dict(torch.load(best_model_path))
mean_loss_val, mean_metrics = val_loop(
    model, 
    ellip_loader_val, 
    binarize_mask, 
...
mean_loss_val, mean_metrics = val_loop(
    ...
    multiclass_watershed_decoder, 
    compute_instance_metrics_wrapper, 
    dim=1
)
...
mean_loss_val, mean_metrics = val_loop(
    ...
    val_dataloader, 
    border_mask, 
    FocalLoss(weight=weight, gamma=1), 
    multiclass_watershed_decoder, 
    compute_instance_metrics_wrapper, 
    dim=1
)
...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = ModUNet(in_channels=3, out_channels=3).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
weight = get_weights(loader=train_dataloader, classes=3, transform_mask=border_mask)
model.load_state_dict(torch.load(best_model_path))
mean_loss_val, mean_metrics = val_loop(
